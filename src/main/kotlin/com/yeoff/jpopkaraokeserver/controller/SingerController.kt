...
class SingerController(val singerService: SingerService) {
    ...
    @GetMapping("/singer")
    fun getSingerList(pageable: Pageable): ResponseEntity<SuccessRespDto<List<SingerListRespDto>>> {
        return ResponseEntity.ok(singerService.getSingerList(pageable))
    }

    @GetMapping("/singer/{singerId}/song")
    fun getSingerSongList(@PathVariable("singerId") singerId: Long, pageable: Pageable)
